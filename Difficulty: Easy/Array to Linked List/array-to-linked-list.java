...
class Solution {
    static Node constructLL(int arr[]) {
        if (arr.length == 0) return null; // Handle the case of an empty array
        
        Node head = new Node(arr[0]); // Create the head node
        Node curr = head; // Initialize the current node to head
        
        for (int i = 1; i < arr.length; i++) {
            Node temp = new Node(arr[i]); // Create a new node with current array element
